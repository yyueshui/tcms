...
class Goods extends Model
{
	...
	public function scopeGetWechatGoodsInfos($query, $name, $uid = '')
	{
		...
		$row = $query->where('goods_name', 'like', '%' . $name . '%')
			...
			->get(['local_image', 'goods_name', 'goods_image', 'tao_short_url', 'tao_password', 'coupon_denomination', 'coupon_password', 'coupon_short_url', 'mouth_sell_number', 'price'])->first();
	...
	public function save(array $options = [])
	{
		...
		$fileName = storage_path(config('app.taoke.images')) . '/' . $name;
		//保存远程图片
		Image::make($this->attributes['goods_image'])
				->save($fileName);
		$this->attributes['local_image'] = $fileName;
